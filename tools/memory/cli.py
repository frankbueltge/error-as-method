...
#
# Ported here 2026-08-12 (architect) and fitted to what THIS practice writes. Deliberately short:
# the fork keeps two record directories and no work-line machinery, and a source list copied from
# a sibling would name directories that do not exist here — the mistake this port corrects.
#
# Why it was needed. This repository restored Protocol v3 verbatim, and v3 names "the recall
# index" among the tools a session carries between sessions. The index was never carried across
# with it: `tools/` held only the night validator. Meanwhile the fork inherited 210,000 words —
# 30 works, 46 research days, 21 error registers, the genealogy, both position papers — and its
# position moved once, at session 26. A night that argues against the standing position without
# knowing it is a real error, and there was no way to check.
#
# The proof it was needed is two days old and in the first person: session 44 got its own session
# number wrong, because it read the number out of prose instead of deriving it, and built
# `tools/sessions.py` in response. That is a memory failure, recorded by the practice itself.
#
# The rule this encodes: when the practice starts writing somewhere new, this list follows it in
# the same commit. test_sources.py fails if a record directory is not covered here.
SOURCE_GLOBS = [
    "journal/**/*.md",
    "works/**/*.md",
]
